...
gulp.task('build', [
  ...
  tasks.push(function (callback) {
    ...
    return gulp.src(config.path.tmpDist + 'config/styles.styl')
...
gulp.task('webpack:dev-server', function (callback) {
  del(config.path.tmp).then(function () {
    ...
    setupIndex(config.path.tmp, function() {}, 'dev');
    copyAssets(config.path.tmp)
    setupModules(function() {});

    const server = new WebpackDevServer(compiler, config.webpack.devServer).listen(8080, "localhost", function(err) {
...
gulp.task('dev:server', [
  ...
  //   res.sendFile(config.path.tmp + 'index.html');
  // });
  //
...
gulp.task('eslint', function () {
  return gulp.src(['src/**/*.js', '!src/bundles/**', '!src/vendors/**'])
    .pipe(eslint())
    .pipe(eslint.format());
});
...
function copyAssets(dest) {
  gulp.src(config.path.assets + '/assets/**')
...
function setupModules(callback) {
  var json = config.path.src + 'modules.json',
    js = config.path.src + 'bundles/base/modules/modules.js';
  try {
    var modules = _.map(loadJSON(json), function (value, key) {
      return `
        "${key}": function(context, params, route, el) {
          import('${value}')
            .then(module => {
...
function showError(title, type, description) {
  notifier.notify({
    title: title,
    message: type + ' error',
    icon: __dirname + '/docs/fw/dan.png'
  });
